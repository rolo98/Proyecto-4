...
Original file is located at
    https://colab.research.google.com/drive/1T6vTCfDJX6a6kZUgn6bm7EPze0e7qO1P
...
* Estudiante: **Rolando Murillo Pérez**
* Carné: **B54966**
* Grupo: **1/2**
...
#Se extraen los pixeles de la imagen
from PIL import Image
import numpy as np
...
def modulador(bits, fc, mpp):
    ...
    :return: La onda cuadrada moduladora (información)
    '''
    CombinBits = bits.reshape(-1, 4)

    # 1. Parámetros de la 'señal' de información (bits)
    N = len(bits)  # Cantidad de bits

    # 2. Construyendo un T para la señal portadora in Phase "I".
    Tc = 1 / fc  #Periodo de la portadora I.
    ...
    moduladora = np.zeros(t_simulacion.shape)  # señal de informació
    ...
    for CombinBits1, CombinBits2,CombinBits3,CombinBits4 in CombinBits:
        #4 if onda portadora I
        if CombinBits1 == 0 and CombinBits2 == 0:
            senal[i * mpp: (i + 2) * mpp] = portadora_I * -3
            moduladora[i * mpp: (i + 1) * mpp] = -3
        if CombinBits1  == 0 and CombinBits2 == 1:
            senal[i * mpp: (i + 2) * mpp] = portadora_I * -1
            moduladora[i * mpp: (i + 1) * mpp] = -1
        if CombinBits1 == 1 and CombinBits2 == 1:
            senal[i * mpp: (i + 2) * mpp] = portadora_I * 1
            moduladora[i * mpp: (i + 1) * mpp] = 1
        if CombinBits1  == 1 and CombinBits2 == 0:
            senal[i * mpp: (i + 2) * mpp] = portadora_I * 3
            moduladora[i * mpp: (i + 1) * mpp] = 3

        # 4 if onda portadoras Q
        if CombinBits3 == 0 and CombinBits4 == 0:
            senal[(i+2) * mpp: (i + 4) * mpp] = portadora_Q * 3
            moduladora[i * mpp: (i + 1) * mpp] = 3

        if CombinBits3== 0 and CombinBits4 == 1:
            senal[(i+2) * mpp: (i + 4) * mpp] = portadora_Q * 1
            moduladora[i * mpp: (i + 1) * mpp] = 0

        if CombinBits3== 1 and CombinBits4 == 1:
            senal[(i+2) * mpp: (i + 4) * mpp] = portadora_Q * -1
            moduladora[i * mpp: (i + 1) * mpp] = -1

        if CombinBits3 == 1 and CombinBits4 == 0:
            senal[(i+2) * mpp: (i + 4) * mpp] = portadora_Q * -3
            moduladora[i * mpp: (i + 1) * mpp] = -3
...
# 1. Importar y convertir la imagen a trasmitir
imagen_Tx = fuente_info('arenal.jpg')#probé con una imagen tomada con el celular pero dura mucho, hubiera sido gracioso usar otra imagen pero luego se enojan jaja
dimensiones = imagen_Tx.shape
...
NUM = 200			# número de elementos (usamos solo 200, no el total de 500)
t_final = 500	# tiempo en segundos
t = np.linspace(0, t_final, NUM)

P = [np.mean(senal_Tx[i]) for i in range(150)]  
